...
Предлагается на примере шаблона из одного учебного курса, на удобном вам фреймворке реализовать ЧАСТЬ функционала, которая продемонстрирует ваши навыки. Например конструктор пиццы, авторизацию, переключение между страницами. Все задание реалзизвывать не надо
