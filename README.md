...
https://github.com/Sense-Capital/TestTemplate

Бек можно не использовать а просто поставить заглушки, но чтобы продемонстировать навыки подключения бека, можно развернуть какой-то простой бек для одной-двух операций.


Результат должен быть представлен в виде Github репозитория с решением и docker-compose который бы запускал решение.
